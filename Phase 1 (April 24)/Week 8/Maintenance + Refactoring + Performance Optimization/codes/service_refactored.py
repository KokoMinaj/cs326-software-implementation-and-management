...
def _process_checked_in_serving(institution, now):
    """Transition SERVING entries with check-ins to SERVED."""
    ...
    return checked_in_serving


def _calculate_new_serving_number(active_entries, randomize):
    """Calculate the next number the institution is serving."""
    ...
    new_serving = max(clamped_current_serving, capped_current_serving)
    return new_serving, increment


def _process_newly_serving(institution, new_serving, now, grace_period_seconds):
    """Transition WAITING entries to SERVING status."""
    newly_serving_entries = list(
        QueueEntry.objects.select_for_update().filter(
            institution=institution,
            status__in=(QueueEntryStatus.WAITING, QueueEntryStatus.NOTIFIED),
            queue_number__lte=new_serving,
        )
    )
    ...
    return newly_serving_entries


def _process_near_turn_notifications(institution, new_serving, now):
    """Notify users who are close to their turn."""
    near_turn_entries = list(
        ...
        .filter(
            ...
            queue_number__gt=new_serving,
        )
        .annotate(
            people_ahead_calc=ExpressionWrapper(
                F("queue_number") - Value(new_serving) - Value(1),
...
def simulate_queue_tick_for_institution(
    ...
    with transaction.atomic():
        ...
        # 1. Expire stale no-shows
        expired_entries = expire_stale_serving_entries(
            institution_id, grace_period_seconds
        )

        # 2. Auto-serve checked-in entries
        served_entries = _process_checked_in_serving(institution, now)

        # 3. Calculate and advance serving number
        active_entries = list(
            QueueEntry.objects.select_for_update()
            .filter(institution=institution, status__in=ACTIVE_QUEUE_STATUSES)
        ...
        if not active_entries:
            ...
            return {
                ...
                "increment": 0,
                "current_serving_number": last_known,
                "served_count": len(served_entries),
                "expired_count": len(expired_entries),
                "message": "No active entries.",
            }

        new_serving, increment = _calculate_new_serving_number(
            active_entries, randomize
        )

        QueueEntry.objects.filter(
            institution=institution, status__in=ACTIVE_QUEUE_STATUSES
        ).update(current_serving_number=new_serving, updated_at=now)

        # 4. Transition and Notify
        newly_serving = _process_newly_serving(
            institution, new_serving, now, grace_period_seconds
        )
        near_turn = _process_near_turn_notifications(institution, new_serving, now)

        return {
            "institution_id": institution.id,
            "randomized": randomize,
            "increment": increment,
            "current_serving_number": new_serving,
            "served_count": len(served_entries),
            "newly_serving_count": len(newly_serving),
            "notified_count": len(near_turn),
            "expired_count": len(expired_entries),
        }
